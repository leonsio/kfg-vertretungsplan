...
// Official teacher abbreviations/names from the KFG Kollegium page.
// Unknown abbreviations deliberately fall back to the abbreviation from Untis.
const TEACHERS = {
  ...
  SMI: 'S. Schmitt', SLZ: 'M. Scholz', SHO: 'F. Schossau', SRB: 'A. Schreiber', SST: 'J. Schuster',
  ...
  ZNN: 'A. Zinn', ZSI: 'A. Zsilinszky',
  BMN: 'S. Baumann', FRY: 'V. Frey', HST: 'M.J. Hastrich', LBU: 'F. Lebeau', MND: 'F. Meinders',
  SWZ: 'E. Schweitzer', STC: 'L. Stachel', STN: 'M. Suton', TRV: 'L. Trovato', WLS: 'M. Walsh', WLL: 'B. Will'
};
...
class KfgVertretungsplanCard extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .subtitle { margin-top:5px; color:var(--secondary-text-color); font-size:.9rem; }
        ...
        .fixed { color:var(--secondary-text-color); font-size:.82rem; margin:4px; }
        .day { margin:10px 0 16px; border:1px solid var(--divider-color); border-radius:12px; overflow:hidden; }
        .day-header { padding:11px 13px; background:var(--secondary-background-color); display:flex; justify-content:space-between; align-items:center; gap:10px; }
        .day-name { font-weight:700; }
        .day-date { color:var(--secondary-text-color); font-size:.88rem; white-space:nowrap; }
        .week-type { font-weight:800; margin-left:10px; }
        .week-a { color:var(--primary-color); }
        .week-b { color:var(--accent-color, var(--primary-color)); }
        .news { margin:10px; padding:10px 12px; border-radius:9px; background:var(--info-color, var(--secondary-background-color)); border-left:4px solid var(--primary-color); }
        .news-title { font-weight:700; margin-bottom:4px; }
        .news p { margin:3px 0; }
        .table-wrap { overflow-x:auto; }
        table { width:100%; border-collapse:collapse; font-size:.88rem; }
        th { text-align:left; color:var(--secondary-text-color); font-weight:600; font-size:.75rem; padding:8px 10px; border-bottom:1px solid var(--divider-color); white-space:nowrap; }
        td { padding:9px 10px; border-bottom:1px solid var(--divider-color); vertical-align:top; }
        tr:last-child td { border-bottom:0; }
        tr.art-entfall { background:rgba(244,67,54,.10); }
        tr.art-vertretung { background:rgba(33,150,243,.08); }
        tr.art-betreuung { background:rgba(76,175,80,.10); }
        tr.art-tausch { background:rgba(255,152,0,.11); }
        tr.art-raumänderung { background:rgba(156,39,176,.09); }
        .klasse { font-weight:700; white-space:nowrap; }
        .lesson { white-space:nowrap; font-weight:700; }
        .art { white-space:nowrap; font-weight:600; }
        .empty { padding:18px; color:var(--secondary-text-color); text-align:center; }
        @media (max-width: 700px) { th:nth-child(4), td:nth-child(4) { display:none; } th,td { padding:8px 7px; } }
      </style>
      <ha-card>
        <div class="header">
        ...
        <div class="section">
          ...
          ${fixed ? `<div class="fixed">Fest konfiguriert: <strong>${configuredClasses.map(this._escape).join(', ')}</strong></div>` : `<div class="classes"><button class="${selected.size === 0 ? 'selected' : ''}" data-all>Alle Klassen</button>${classes.map(c => `<button class="${selected.has(c) ? 'selected' : ''}" data-class="${this._escape(c)}">${this._escape(c)}</button>`).join('')}</div>`}
        </div>
        <div class="section">${days.map(day => this._renderDay(day)).join('') || '<div class="empty">Keine Vertretungen für die ausgewählten Klassen vorhanden.</div>'}</div>
      </ha-card>
    `;
  ...
  _renderDay(day) {
    ...
    const news = (day.news || []).filter(n => String(n).trim());
    const weekClass = String(day.weekType || '').toLowerCase().includes('b') ? 'week-b' : 'week-a';
    return `
      <div class="day">
        <div class="day-header">
          <div><span class="day-name">${this._escape(day.weekday || '')}</span><span class="week-type ${weekClass}">${this._escape(day.weekType || '')}</span></div>
          <div class="day-date">${this._escape(day.date || '')}</div>
        </div>
        ${news.length ? `<div class="news"><div class="news-title">Nachricht des Tages</div>${news.map(n => `<p>${this._escape(this._newsText(n))}</p>`).join('')}</div>` : ''}
        ${entries.length ? `<div class="table-wrap"><table><thead><tr><th>Klasse</th><th>Stunde</th><th>Fach</th><th>Lehrer</th><th>Vertretung</th><th>Raum</th><th>Art</th></tr></thead><tbody>${entries.map(e => this._renderEntry(e)).join('')}</tbody></table></div>` : `<div class="empty">Keine Vertretungen</div>`}
      </div>`;
  }

  _renderEntry(e) {
    const art = this._label(e.art);
    const cls = `art-${art.toLowerCase().replace(/[^a-zäöüß]+/g, '-')}`;
    return `<tr class="${cls}"><td class="klasse">${this._escape(e.klasse || '–')}</td><td class="lesson">${this._escape(e.stunde || '–')}</td><td>${this._escape(this._subject(e.fach))}</td><td>${this._escape(this._teacher(e.lehrer_original))}</td><td>${this._escape(this._teacher(e.vertreter))}</td><td>${this._escape(e.raum || '–')}</td><td class="art">${this._escape(art)}</td></tr>`;
  }
  ...
  _teacher(value) {
    ...
    return TEACHERS[key] ? `${TEACHERS[key]} (${raw})` : raw;
  }

  _subject(value) {
    if (!value) return '–';
    const raw = String(value).trim();
    return SUBJECTS[raw.toUpperCase()] ? `${SUBJECTS[raw.toUpperCase()]} (${raw})` : raw;
  }
  ...
  _newsText(value) {
    ...
    if (value && typeof value === 'object') return value.text || value.news || value.message || JSON.stringify(value);
    return String(value ?? '');
  }
...
if (!window.customCards.some(c => c.type === CARD_NAME)) window.customCards.push({ type:CARD_NAME, name:'KFG Vertretungsplan', description:'Vertretungsplan des Kaiserin-Friedrich-Gymnasiums' });
